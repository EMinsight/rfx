...
def test_drawn_counts_are_the_realized_counts(fixture):
    """t_c = round(t/dx) and L_c = round(L/dx) — no compensation left.

    The deleted rule was ``t_c = round(t/dx) + 1`` and ``L_c = round(L/dx) - 1``:
    the builder drew a thicker iris and a shorter cavity so that a realization
    which lost one wall per region would land the electrical dimensions on
    nominal. Two errors chosen to cancel. Now the drawing is the geometry, and
    the only residual is the honest one, half a cell of rounding onto the
    lattice.
    """
    _require_post_931(fixture)
    eg = fixture["electrical_geometry"]
    ...
    cfg = fixture["config"]
    dx = A / cfg["gated_cells_per_a"]
    assert eg["iris_thickness_cells"] == round(
        ref["iris_thickness_mm"] * 1e-3 / dx)
    assert list(eg["cavity_cells"]) == [
    ...
    for (lo, hi), d_mm in zip(fixture["gated_rfx"]["aperture_nodes"],
        ...
        # the producer rounds onto the lattice, so the bound is dx/2. A full-dx
        # bound once admitted an outer-aperture +1-node mutation that would
        # have improved d_lo from 17.08 to 6.76 MHz.
...
def _aps_offs(fixture, dx):
    ...
    #931: ``aperture_nodes`` are the OPEN nodes between two fins, so the fins'
    inner faces -- both of them walls under the contract -- are at ``lo - 1``
    and ``hi + 1``, and the clear aperture is the distance between those two
    wall planes. The arithmetic below is unchanged in form and now says that
    rather than counting a face the old rule left live.
    """
    aps, offs = [], []
...
def test_setup_conventions_are_content_pinned(script_src):
    ...
    #931: three of the pinned phrases named the deleted compensation --
    "bounding zeroed node planes", "round(t/dx) + 1", "round(L/dx) - 1". They
    are pinned ABSENT now, so the compensation cannot come back by a quiet
    edit, and the builder is required instead to read its geometry back
    through the one realized-edge function.
    """
    for phrase in (
            "NOT a monotone",
    ...
    for required in ("realized_wall_planes", "#931"):
        assert required in script_src, (
            ...
            "must be read back through the one realized-edge function, not "
            "re-derived from cell arithmetic (design note §1.7)")
