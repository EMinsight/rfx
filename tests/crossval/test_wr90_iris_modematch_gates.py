...
def test_operating_point_is_grid_exact_on_every_row(fixture):
    ...
    for r in rows:
        ...
        # drawn gap. The old ``d_c - 1`` counted one fin face the pre-#931 rule
        # never zeroed; it is deleted, not re-tuned.
        if _fixture_is_post_931(fixture):
            assert r["aperture_cells"] == d_c, r
        else:
...
def test_the_two_wr90_iris_cases_share_one_thickness_convention(fixture):
    ...
    for r in _rows(fixture):
        assert r["thickness_cells"] == round(
...
def test_the_one_cell_iris_is_witnessed(fixture):
    ...
    """
    rows = [r for r in _rows(fixture) if int(r["thickness_cells"]) == 1]
    if not rows:
        pytest.skip(
