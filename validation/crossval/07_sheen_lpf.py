...
!!! SUPERSEDED FRAMING — read the Palace referee before trusting the "first null"
    ...
    #931 LATTICE OWNERSHIP — APPENDED 2026-09-07. Nothing above is
    withdrawn; the paragraphs this supersedes are marked below.
    ...
    Re(Z0) this case measures at 50.30 ohm. That would put a 4-ohm error in
    the extractor's reference to fix a one-row aperture — a compensation,
    not a fix. The real remedy is §1.9's: the MSL port takes its
    ...
    COMMITTED below was measured on the pre-#931 board (metal one cell
    wider on every in-plane dimension AND a 200 um slab in z) and is
    regenerated by this change; the openEMS half does not move (external
    solver, true zero-thickness sheet, declared board). C4's +-0.50%
...
def build_rfx_sim(dx):
    ...
    # reference at HJ = 54.22 ohm on a line this case measures at 50.30 ohm.
