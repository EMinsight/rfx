...
Grid sizes tested: 32^3, 64^3, 128^3, 200^3

2-panel figure:
  Panel 1 — Throughput (Mcells/s) vs grid size (log x-scale)
             Annotated with peak throughput value.
  Panel 2 — Wall time (s) vs grid size
             Grouped bars for execution time and JIT+exec time.

Note: GPU numbers require a GPU node. CPU results are valid for
relative scaling comparisons. Actual throughput is runtime-dependent.
...
GRID_SIZES = [32, 64, 128, 200]   # cubic grid edge lengths

DX = 1e-3       # 1 mm cell size
N_STEPS = 100   # fixed step count for fair comparison
F_MAX = 10e9    # 10 GHz (sets dt via CFL)
F0 = 5e9        # source centre frequency


def _run_one(n: int, n_steps: int) -> dict:
    """Benchmark a single N^3 grid.

    Returns dict with keys: n, shape, n_cells, jit_time_s, exec_time_s,
    ...
    # ---- Timed execution (second call — already compiled) ----
    ...
    mcells_per_s = n_cells * n_steps / exec_time / 1e6
...
print()
print(f"{'Grid':>8s}  {'Cells':>10s}  {'JIT (s)':>9s}  "
      f"{'Exec (s)':>9s}  {'Mcells/s':>10s}")
print("-" * 57)
...
for n in GRID_SIZES:
    ...
    print(f"{str(res['shape']):>10s}  {res['n_cells']:>10,}  "
...
jit_tp      = n_cells_arr * N_STEPS / jit_times / 1e6   # JIT-inclusive throughput

peak_idx = int(np.argmax(throughputs))
peak_tp  = throughputs[peak_idx]
peak_n   = sizes[peak_idx]

print(f"Peak throughput: {peak_tp:.1f} Mcells/s at {peak_n}^3 grid")

# ---------------------------------------------------------------------------
# Figure: 2-panel
# ---------------------------------------------------------------------------

x_labels = [f"{n}^3" for n in sizes]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle(
    f"rfx FDTD Benchmark — {backend.upper()}  ({N_STEPS} steps/run)",
    fontsize=13,
)

# -- Panel 1: Throughput (Mcells/s) vs grid size (log x-axis) -------------
ax1.semilogx(n_cells_arr, throughputs, "o-", color="#1f77b4",
             linewidth=2, markersize=8, markerfacecolor="white",
             markeredgewidth=2, label="Execution only")
...
ax1.annotate(
    ...
    xytext=(n_cells_arr[peak_idx] * 1.4, peak_tp * 0.82),
    fontsize=9,
...
ax1.set_xlabel("Grid cells (N^3, log scale)")
...
for bar in bars_exec:
    ...
    ax2.text(bar.get_x() + bar.get_width() / 2,
             h + max(exec_times) * 0.01,
             f"{h:.2f}s", ha="center", va="bottom", fontsize=8)
for bar in bars_jit:
    h = bar.get_height()
    ax2.text(bar.get_x() + bar.get_width() / 2,
             h + max(jit_times) * 0.01,
             f"{h:.2f}s", ha="center", va="bottom", fontsize=8)
